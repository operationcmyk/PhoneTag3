...
final class MockGameRepository: GameRepositoryProtocol {
    ...
    init() {
        ...
        activePlayer1.homeBase = CLLocationCoordinate2D(latitude: 40.7128, longitude: -74.0060)
        activePlayer1.strikes = 2

        var activePlayer2 = defaultPlayerState
        activePlayer2.homeBase = CLLocationCoordinate2D(latitude: 40.7580, longitude: -73.9855)
    ...
    func submitTag(
        ...
        for (playerId, playerState) in game.players where playerId != fromUserId && playerState.isActive {
            ...
            // Check if target is at home base
            if let homeBase = playerState.homeBase {
