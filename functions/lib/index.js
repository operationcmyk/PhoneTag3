...
exports.sendNotification = functions.https.onCall(async (data, context) => {
    ...
    const { title, body, data: notifData, token, tokens } = data;
    if (!title || !body) {
    ...
    if (!token && (!tokens || Object.keys(tokens).length === 0)) {
        throw new functions.https.HttpsError("invalid-argument", "Provide either 'token' (single recipient) or 'tokens' (multiple recipients).");
    }
    const apns = {
        payload: {
            aps: {
