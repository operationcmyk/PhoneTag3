...
// }
// ---------------------------------------------------------------------------
export const nudgePlayers = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "You must be signed in to nudge players."
    );
  }

  const { gameId, gameTitle, nudgedByName, recipientTokens } = data as {
    gameId: string;
    gameTitle: string;
    nudgedByName: string;
    recipientTokens: Record<string, string>;
  ...
  if (!gameId || !gameTitle || !nudgedByName || !recipientTokens) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "Missing required fields: gameId, gameTitle, nudgedByName, recipientTokens"
    );
  }
...
// }
// ---------------------------------------------------------------------------
export const sendTagWarning = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      "unauthenticated",
  ...
  const { recipientToken, taggerName, gameTitle } = data as {
  ...
  if (!recipientToken || !taggerName || !gameTitle) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "Missing required fields: recipientToken, taggerName, gameTitle"
    );
  }
